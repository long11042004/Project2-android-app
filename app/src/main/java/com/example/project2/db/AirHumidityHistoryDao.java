...
public interface AirHumidityHistoryDao {
    ...
    @Query("SELECT * FROM air_humidity_history WHERE timestamp >= :startTime AND timestamp <= :endTime ORDER BY timestamp ASC")
    List<AirHumidityHistoryEntry> getHistoryInRange(long startTime, long endTime);
